...
export default class WTCTorqueCorrectionCalculating extends Component {
  constructor(props){
    super(props)
  ...
  render() {
    ...
    var ID = "WTCTorqueCorrectionCalculating" + Math.trunc(Math.random()*1000) + Math.trunc(Math.random()*1000)
    ...
    return(
      <g>
        <defs>
          <g id={ID}>
            <rect width="144" height="36" fill="none" />
            <line x1="4" y1="18" x2="140" y2="18" />
            <text x="30" y="12" style={text}>reCorrectionTP * reRadiusTP</text>
            <text x="32" y="27" style={text}>reNormalizedRadiusMaxTP</text>
          </g>
        </defs>
        <use x={this.props.x} y={this.props.y} href={ '#' + ID } style={style(this.state.green)} />
        <rect x={this.props.x} y={this.props.y} width="144" height="36" fill="transparent" cursor="pointer" onClick={() => this.setState({toggle: !this.state.toggle})} />
...
WTCTorqueCorrectionCalculating.defaultProps = {
  x: 0,
  y: 0,
  green: false,
  textPosOffsetX: 0,
...
WTCTorqueCorrectionCalculating.propTypes = {
  x: PropTypes.number,
  y: PropTypes.number,
